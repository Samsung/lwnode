...
// @lwnode using CompressibleString occurs memory leak
static bool useCompressibleString = false;
...
#if defined(ESCARGOT_ENABLE_TEST_)
...
// <---- these header & function above are used for Escargot internal
// development
#endif
...
static OptionalRef<StringRef> builtinHelperFileRead(
  ...
  if (fp) {
    ...
    fclose(fp);
    if (useCompressibleString && StringRef::isCompressibleStringEnabled()) {
      if (state) {
        if (hasNonLatin1Content) {
        ...
        if (hasNonLatin1Content) {
          src = StringRef::createFromUTF8(
              utf8Str.data(), utf8Str.length(), false);
        } else {
      ...
      if (hasNonLatin1Content) {
        src =
            StringRef::createFromUTF8(utf8Str.data(), utf8Str.length(), false);
      } else {
        src = StringRef::createFromLatin1(str.data(), str.length());
      }
    }
...
static ValueRef* builtinUneval(ExecutionStateRef* state,
                               ValueRef* thisValue,
                               size_t argc,
                               ValueRef** argv,
                               bool isConstructCall) {
...
static ValueRef* builtinDrainJobQueue(ExecutionStateRef* state,
                                      ValueRef* thisValue,
                                      size_t argc,
                                      ValueRef** argv,
                                      bool isConstructCall) {
...
static ValueRef* builtinAddPromiseReactions(ExecutionStateRef* state,
                                            ValueRef* thisValue,
                                            size_t argc,
                                            ValueRef** argv,
                                            bool isConstructCall) {
  if (argc >= 3) {
    ...
    state->throwException(
        TypeErrorObjectRef::create(state, StringRef::emptyString()));
  }
  return ValueRef::createUndefined();
}

static ValueRef* builtinCreateNewGlobalObject(ExecutionStateRef* state,
                                              ValueRef* thisValue,
                                              size_t argc,
                                              ValueRef** argv,
                                              bool isConstructCall) {
...
static ValueRef* builtin262CreateRealm(ExecutionStateRef* state,
                                       ValueRef* thisValue,
                                       size_t argc,
                                       ValueRef** argv,
                                       bool isConstructCall) {
  auto newContext =
      createEscargotContext(state->context()->vmInstance(), false);
  return newContext->globalObject()->get(state,
                                         StringRef::createFromASCII("$262"));
}

static ValueRef* builtin262DetachArrayBuffer(ExecutionStateRef* state,
                                             ValueRef* thisValue,
                                             size_t argc,
                                             ValueRef** argv,
                                             bool isConstructCall) {
...
static ValueRef* builtin262EvalScript(ExecutionStateRef* state,
                                      ValueRef* thisValue,
                                      size_t argc,
                                      ValueRef** argv,
                                      bool isConstructCall) {
  ...
  auto script =
      state->context()
          ->scriptParser()
          ->initializeScript(
              src, StringRef::createFromASCII("$262.evalScript input"), false)
          .fetchScriptThrowsExceptionIfParseError(state);
  return script->execute(state);
}

static ValueRef* builtin262IsHTMLDDA(ExecutionStateRef* state,
                                     ValueRef* thisValue,
                                     size_t argc,
                                     ValueRef** argv,
                                     bool isConstructCall) {
...
static ValueRef* builtin262AgentStart(ExecutionStateRef* state,
  ...
  std::thread worker(
      [](std::string script) {
        ...
        PersistentRefHolder<ContextRef> context =
            createEscargotContext(instance.get(), false);

        evalScript(context.get(),
                   StringRef::createFromUTF8(script.data(), script.size()),
                   StringRef::createFromASCII("from main thread"),
                   false,
                   false);
        ...
        while (true) {
          {
            ...
            for (size_t i = 0; i < workerThreads.size(); i++) {
              if (workerThreads[i].first.get_id() ==
          ...
          if (message.length()) {
            ...
            ValueRef* val1 =
                SerializerRef::deserializeFrom(context.get(), istream);
            ValueRef* val2 =
                SerializerRef::deserializeFrom(context.get(), istream);

            ValueRef* callback =
                (ValueRef*)context.get()->globalObject()->extraData();
            if (callback) {
              Evaluator::execute(
                  context.get(),
                  [](ExecutionStateRef* state,
                     ValueRef* callback,
                     ValueRef* v1,
                     ValueRef* v2) -> ValueRef* {
                    ValueRef* argv[2] = {v1, v2};
                    return callback->call(
                        state, ValueRef::createUndefined(), 2, argv);
                  },
                  callback,
                  val1,
                  val2);
...

  {
    ...
    workerThreads.push_back(
        std::make_pair(std::move(worker), WorkerThreadData()));
  }

  return ValueRef::createUndefined();
}

static ValueRef* builtin262AgentBroadcast(ExecutionStateRef* state,
                                          ValueRef* thisValue,
                                          size_t argc,
                                          ValueRef** argv,
                                          bool isConstructCall) {
...
static ValueRef* builtin262AgentReport(ExecutionStateRef* state,
                                       ValueRef* thisValue,
                                       size_t argc,
                                       ValueRef** argv,
                                       bool isConstructCall) {
...
static ValueRef* builtin262AgentLeaving(ExecutionStateRef* state,
                                        ValueRef* thisValue,
                                        size_t argc,
                                        ValueRef** argv,
                                        bool isConstructCall) {
...
static ValueRef* builtin262AgentReceiveBroadcast(ExecutionStateRef* state,
                                                 ValueRef* thisValue,
                                                 size_t argc,
                                                 ValueRef** argv,
                                                 bool isConstructCall) {
...
static ValueRef* builtin262AgentGetReport(ExecutionStateRef* state,
                                          ValueRef* thisValue,
                                          size_t argc,
                                          ValueRef** argv,
                                          bool isConstructCall) {
...
static ValueRef* builtin262AgentMonotonicNow(ExecutionStateRef* state,
                                             ValueRef* thisValue,
                                             size_t argc,
                                             ValueRef** argv,
                                             bool isConstructCall) {
...
static ValueRef* builtin262AgentSleep(ExecutionStateRef* state,
                                      ValueRef* thisValue,
                                      size_t argc,
                                      ValueRef** argv,
                                      bool isConstructCall) {
...
class ShellPlatform : public PlatformRef {
 public:
  ...
  ShellPlatform() : m_canBlock(true) {}

  void setCanBlock(bool b) { m_canBlock = b; }
  ...
  virtual LoadModuleResult onLoadModule(ContextRef* relatedContext,
    ...
    auto& loadedModules = *reinterpret_cast<std::vector<
        std::tuple<std::string, ContextRef*, PersistentRefHolder<ScriptRef>>>*>(
        threadLocalCustomData());
    ...
    auto parseResult = relatedContext->scriptParser()->initializeScript(
        source.value(),
        StringRef::createFromUTF8(absPath.data(), absPath.size()),
        true);
  ...
  virtual void didLoadModule(ContextRef* relatedContext,
    ...
    std::string path;
    if (referrer && loadedModule->src()->length() &&
        loadedModule->src()->charAt(0) != '/') {
  ...
  virtual void* allocateThreadLocalCustomData() override {
    return new std::vector<std::tuple<std::string /* abs path */,
                                      ContextRef*,
                                      PersistentRefHolder<ScriptRef>>>();
  }

  virtual void deallocateThreadLocalCustomData() override {
    delete reinterpret_cast<std::vector<
        std::tuple<std::string, ContextRef*, PersistentRefHolder<ScriptRef>>>*>(
        threadLocalCustomData());
  }
 ...
 private:
  ...
  std::string absolutePath(const std::string& referrerPath,
...
static bool evalScript(ContextRef* context,
  ...
  auto scriptInitializeResult =
      context->scriptParser()->initializeScript(source, srcName, isModule);
  ...
  if (!evalResult.isSuccessful()) {
    ...
    for (size_t i = 0; i < evalResult.stackTrace.size(); i++) {
      fprintf(stderr,
              "%s (%d:%d)\n",
              evalResult.stackTrace[i].srcName->toStdUTF8String().data(),
              (int)evalResult.stackTrace[i].loc.line,
              (int)evalResult.stackTrace[i].loc.column);
    }
    return false;
  }
...
PersistentRefHolder<ContextRef> createEscargotContext(VMInstanceRef* instance,
  ...
  Evaluator::execute(
      context,
      [](ExecutionStateRef* state, bool isMainThread) -> ValueRef* {
        ContextRef* context = state->context();

        GlobalObjectProxyObjectRef* proxy = GlobalObjectProxyObjectRef::create(
            state,
            state->context()->globalObject(),
            [](ExecutionStateRef* state,
               GlobalObjectProxyObjectRef* proxy,
               GlobalObjectRef* targetGlobalObject,
               GlobalObjectProxyObjectRef::AccessOperationType operationType,
        ...
        {
          FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
              AtomicStringRef::create(context, "run"),
              builtinRun,
              1,
              true,
              false);
        ...
        {
          FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
              AtomicStringRef::create(context, "gc"),
              builtinGc,
              0,
              true,
              false);
...
#if defined(ESCARGOT_ENABLE_TEST)
        // There is no specific standard for the [@@toStringTag] property of
        // global object. But "global" string is added here to pass legacy TCs
        context->globalObject()->defineDataProperty(
            state,
            context->vmInstance()->toStringTagSymbol(),
            ObjectRef::DataPropertyDescriptor(
                AtomicStringRef::create(context, "global")->string(),
                (ObjectRef::PresentAttribute)(ObjectRef::NonWritablePresent |
                                              ObjectRef::NonEnumerablePresent |
                                              ObjectRef::ConfigurablePresent)));

        {
          FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
              AtomicStringRef::create(context, "uneval"),
              builtinUneval,
              1,
              true,
              false);
          ...
          context->globalObject()->defineDataProperty(
              state,
              StringRef::createFromASCII("uneval"),
              buildFunctionObjectRef,
              true,
              true,
              true);
        ...
        {
          FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
              AtomicStringRef::create(context, "drainJobQueue"),
              builtinDrainJobQueue,
              0,
              true,
              false);
          FunctionObjectRef* buildFunctionObjectRef =
          ...
          context->globalObject()->defineDataProperty(
              state,
              StringRef::createFromASCII("drainJobQueue"),
              buildFunctionObjectRef,
              true,
              true,
              true);
        ...
        {
          FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
              AtomicStringRef::create(context, "addPromiseReactions"),
              builtinAddPromiseReactions,
              3,
              true,
              false);
          FunctionObjectRef* buildFunctionObjectRef =
          ...
          context->globalObject()->defineDataProperty(
              state,
              StringRef::createFromASCII("addPromiseReactions"),
              buildFunctionObjectRef,
              true,
              true,
              true);
        ...
        {
          FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
              AtomicStringRef::create(context, "createNewGlobalObject"),
              builtinCreateNewGlobalObject,
              0,
              true,
              false);
          FunctionObjectRef* buildFunctionObjectRef =
          ...
          context->globalObject()->defineDataProperty(
              state,
              StringRef::createFromASCII("createNewGlobalObject"),
              buildFunctionObjectRef,
              true,
              true,
              true);
        ...
        {
          FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
              AtomicStringRef::create(context, "newGlobal"),
              builtinCreateNewGlobalObject,
              0,
              true,
              false);
          ...
          context->globalObject()->defineDataProperty(
              state,
              StringRef::createFromASCII("newGlobal"),
              buildFunctionObjectRef,
              true,
              true,
              true);
        ...
        // https://github.com/tc39/test262/blob/master/INTERPRETING.md
        {
...

          {
            ...
            FunctionObjectRef* buildFunctionObjectRef =
                FunctionObjectRef::create(state, nativeFunctionInfo);
            dollor262Object->defineDataProperty(
                state,
                StringRef::createFromASCII("gc"),
                buildFunctionObjectRef,
                true,
          ...
          {
            FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
                AtomicStringRef::create(context, "createRealm"),
                builtin262CreateRealm,
                0,
                true,
                false);
            FunctionObjectRef* buildFunctionObjectRef =
            ...
            dollor262Object->defineDataProperty(
                state,
                StringRef::createFromASCII("createRealm"),
                buildFunctionObjectRef,
                true,
                true,
                true);
          ...
          {
            FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
                AtomicStringRef::create(context, "detachArrayBuffer"),
                builtin262DetachArrayBuffer,
                1,
                true,
                false);
            FunctionObjectRef* buildFunctionObjectRef =
            ...
            dollor262Object->defineDataProperty(
                state,
                StringRef::createFromASCII("detachArrayBuffer"),
                buildFunctionObjectRef,
                true,
                true,
                true);
          ...
          {
            FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
                AtomicStringRef::create(context, "evalScript"),
                builtin262EvalScript,
                1,
                true,
                false);
            FunctionObjectRef* buildFunctionObjectRef =
            ...
            dollor262Object->defineDataProperty(
                state,
                StringRef::createFromASCII("evalScript"),
                buildFunctionObjectRef,
                true,
                true,
                true);
...

          {
            dollor262Object->defineDataProperty(
                state,
                StringRef::createFromASCII("global"),
                context->globalObjectProxy(),
                true,
                true,
                true);
          }
          ...
          {
            // FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
            //     AtomicStringRef::create(context, "IsHTMLDDA"),
            //     builtin262IsHTMLDDA,
            //     0,
            //     true,
            //     false);
            ...
            // dollor262Object->defineDataProperty(
            //     state,
            //     StringRef::createFromASCII("IsHTMLDDA"),
            //     buildFunctionObjectRef,
            //     true,
            //     true,
            //     true);
          ...
          if (Globals::supportsThreading()) {
            ...
            if (isMainThread) {
              {
                FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
                    AtomicStringRef::create(context, "start"),
                    builtin262AgentStart,
                    1,
                    true,
                    false);
                FunctionObjectRef* buildFunctionObjectRef =
                    FunctionObjectRef::create(state, nativeFunctionInfo);
                agentObject->defineDataProperty(
                    state,
                    StringRef::createFromASCII("start"),
                    buildFunctionObjectRef,
                    true,
                    true,
                    true);
              ...
              {
                FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
                    AtomicStringRef::create(context, "broadcast"),
                    builtin262AgentBroadcast,
                    2,
                    true,
                    false);
                FunctionObjectRef* buildFunctionObjectRef =
                ...
                agentObject->defineDataProperty(
                    state,
                    StringRef::createFromASCII("broadcast"),
                    buildFunctionObjectRef,
                    true,
                    true,
                    true);
              ...
              {
                FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
                    AtomicStringRef::create(context, "getReport"),
                    builtin262AgentGetReport,
                    0,
                    true,
                    false);
                FunctionObjectRef* buildFunctionObjectRef =
                ...
                agentObject->defineDataProperty(
                    state,
                    StringRef::createFromASCII("getReport"),
                    buildFunctionObjectRef,
                    true,
                    true,
                    true);
              ...
              {
                FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
                    AtomicStringRef::create(context, "leaving"),
                    builtin262AgentLeaving,
                    0,
                    true,
                    false);
                FunctionObjectRef* buildFunctionObjectRef =
                ...
                agentObject->defineDataProperty(
                    state,
                    StringRef::createFromASCII("leaving"),
                    buildFunctionObjectRef,
                    true,
                    true,
                    true);
              ...
              {
                FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
                    AtomicStringRef::create(context, "receiveBroadcast"),
                    builtin262AgentReceiveBroadcast,
                    1,
                    true,
                    false);
                FunctionObjectRef* buildFunctionObjectRef =
                ...
                agentObject->defineDataProperty(
                    state,
                    StringRef::createFromASCII("receiveBroadcast"),
                    buildFunctionObjectRef,
                    true,
                    true,
                    true);
              ...
              {
                FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
                    AtomicStringRef::create(context, "report"),
                    builtin262AgentReport,
                    1,
                    true,
                    false);
                FunctionObjectRef* buildFunctionObjectRef =
                ...
                agentObject->defineDataProperty(
                    state,
                    StringRef::createFromASCII("report"),
                    buildFunctionObjectRef,
                    true,
                    true,
                    true);
            ...
            {
              FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
                  AtomicStringRef::create(context, "monotonicNow"),
                  builtin262AgentMonotonicNow,
                  0,
                  true,
                  false);
              FunctionObjectRef* buildFunctionObjectRef =
              ...
              agentObject->defineDataProperty(
                  state,
                  StringRef::createFromASCII("monotonicNow"),
                  buildFunctionObjectRef,
                  true,
                  true,
                  true);
            ...
            {
              FunctionObjectRef::NativeFunctionInfo nativeFunctionInfo(
                  AtomicStringRef::create(context, "sleep"),
                  builtin262AgentSleep,
                  1,
                  true,
                  false);
              FunctionObjectRef* buildFunctionObjectRef =
              ...
              agentObject->defineDataProperty(
                  state,
                  StringRef::createFromASCII("sleep"),
                  buildFunctionObjectRef,
                  true,
                  true,
                  true);
            ...
            dollor262Object->defineDataProperty(
                state,
                StringRef::createFromASCII("agent"),
                agentObject,
                true,
                false,
                true);
...
int entry(int argc, char* argv[]) {
  ...
  PersistentRefHolder<VMInstanceRef> instance = VMInstanceRef::create();
  PersistentRefHolder<ContextRef> context =
      createEscargotContext(instance.get());

  if (getenv("GC_FREE_SPACE_DIVISOR") &&
      strlen(getenv("GC_FREE_SPACE_DIVISOR"))) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strlen(argv[i]) >= 2 &&
        argv[i][0] == '-') {    // parse command line option
      if (argv[i][1] == '-') {  // `--option` case
        ...
        if (strcmp(argv[i], "-e") == 0) {
          ...
          if (!evalScript(context,
                          src,
                          StringRef::createFromASCII("shell input"),
                          false,
                          false))
            return 3;
          continue;
    ...
    if (fp) {
      ...
      StringRef* src =
          Evaluator::execute(
              context,
              [](ExecutionStateRef* state, char* c) -> ValueRef* {
      ...
      if (!evalScript(
              context,
              src,
              StringRef::createFromUTF8(fileName.data(), fileName.length()),
              false,
              seenModule)) {
        return 3;
      }
  ...
  if (runShell && !context->isDebuggerRunning()) {
    printf("escargot version:%s, %s%s\n",
           Globals::version(),
           Globals::buildDate(),
           Globals::supportsThreading() ? "(supports threading)" : "");
  }
  ...
  while (runShell) {
    ...
    evalScript(context,
               str,
               StringRef::createFromASCII("from shell input"),
               true,
               false);
  }
